...
def parseargs():
    ...
    parser.add_argument("--model_dirs", type=str, required=True, nargs='+',
                        help="the model dir names of all checkpoits")
    parser.add_argument("--output", type=str, help="output path")

    return parser.parse_args()


def get_checkpoints(model_dirs):
    checkpoint_names = []
...
def main(_):
    ...
    checkpoints = get_checkpoints(FLAGS.model_dirs)
    ...
    path = FLAGS.model_dirs[0]
    params_pattern = os.path.join(path, "*.json")
    params_files = tf.gfile.Glob(params_pattern)

    for name in params_files:
        new_name = name.replace(path.rstrip("/"),
                                FLAGS.output.rstrip("/"))
